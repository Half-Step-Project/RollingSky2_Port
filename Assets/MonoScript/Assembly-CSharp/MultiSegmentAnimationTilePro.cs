...
public class MultiSegmentAnimationTilePro : BaseTile
{
	[Serializable]
	public struct Data
	{
		...
		public string m_initEffectName;
		...
		public string m_triggerEffectName;
		...
		public string m_motionEffectName;
		...
		public string m_stepOnEffectName;
	...
	public struct RebirthData
	{
		...
		public RD_ElementParticle_DATA initEffect;

		public RD_ElementParticle_DATA triggerEffect;

		public RD_ElementParticle_DATA motionEffect;

		public RD_ElementParticle_DATA stepOnEffect;
	...
	private ParticleSystem m_initEffect;

	private ParticleSystem m_triggerEffect;

	private ParticleSystem m_motionEffect;

	private ParticleSystem m_stepOnEffect;
	...
	public override void Initialize()
	{
		...
		if (m_data.m_isPlayInitAnim && m_animation != null)
		{
			...
			if (m_initEffect != null)
			{
				m_initEffect.Play(true);
			}
		}
	...
	public override void UpdateElement()
	{
		...
		if (m_data.m_isPlayMotionAnim && m_motionAnimTrigger.Run(m_currentDistance) && m_animation != null)
		{
			...
			if (m_motionEffect != null)
			{
				m_motionEffect.Play(true);
			}
		}
		if (m_move != null && m_moveTarget != null)
		{
	...
	public override void ResetElement()
	{
		...
		if (m_initEffect != null)
		{
			m_initEffect.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
		}
		if (m_triggerEffect != null)
		{
			m_triggerEffect.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
		}
		if (m_motionEffect != null)
		{
			m_motionEffect.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
		}
		if (m_stepOnEffect != null)
		{
			m_stepOnEffect.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
		}
	...
	{
		if (m_data.m_isPlayInitAnim && !string.IsNullOrEmpty(m_data.m_initEffectName))
		{
			Transform transform = base.gameObject.transform.Find(m_data.m_initEffectName);
			if (transform != null)
			{
				m_initEffect = transform.gameObject.GetComponent<ParticleSystem>();
				if (m_initEffect != null)
				{
					m_initEffect.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
				}
		...
		if (m_data.m_isPlayTriggerAnim && !string.IsNullOrEmpty(m_data.m_triggerEffectName))
		{
			Transform transform2 = base.gameObject.transform.Find(m_data.m_triggerEffectName);
			if (transform2 != null)
			{
				m_triggerEffect = transform2.gameObject.GetComponent<ParticleSystem>();
				if (m_triggerEffect != null)
				{
					m_triggerEffect.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
				}
		...
		{
			Transform transform3 = base.gameObject.transform.Find(m_data.m_moveTargetName);
			if (transform3 != null)
			{
				m_moveTarget = transform3.gameObject;
			}
		}
		if (m_data.m_isCanRotate && !string.IsNullOrEmpty(m_data.m_rotateTargetName))
		{
			Transform transform4 = base.gameObject.transform.Find(m_data.m_rotateTargetName);
			if (transform4 != null)
			{
				m_rotateTarget = transform4.gameObject;
			}
		}
		if (m_data.m_isPlayMotionAnim && !string.IsNullOrEmpty(m_data.m_motionEffectName))
		{
			Transform transform5 = base.gameObject.transform.Find(m_data.m_motionEffectName);
			if (transform5 != null)
			{
				m_motionEffect = transform5.gameObject.GetComponent<ParticleSystem>();
				if (m_motionEffect != null)
				{
					m_motionEffect.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
				}
		...
		if (m_data.m_isPlayStepOnAnim && !string.IsNullOrEmpty(m_data.m_stepOnEffectName))
		{
			Transform transform6 = base.gameObject.transform.Find(m_data.m_stepOnEffectName);
			if (transform6 != null)
			{
				m_stepOnEffect = transform6.gameObject.GetComponent<ParticleSystem>();
				if (m_stepOnEffect != null)
				{
					m_stepOnEffect.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
				}
	...
	public override byte[] RebirthWriteByteData()
	{
		...
		rebirthData.initEffect = m_initEffect.GetParticleData();
		rebirthData.triggerEffect = m_triggerEffect.GetParticleData();
		rebirthData.motionEffect = m_motionEffect.GetParticleData();
		rebirthData.stepOnEffect = m_stepOnEffect.GetParticleData();
	...
	public override void RebirthReadByteData(byte[] rd_data)
	{
		...
		m_initEffect.SetParticleData(m_rebirthData.initEffect, ProcessState.Pause);
		m_triggerEffect.SetParticleData(m_rebirthData.triggerEffect, ProcessState.Pause);
		m_motionEffect.SetParticleData(m_rebirthData.motionEffect, ProcessState.Pause);
		m_stepOnEffect.SetParticleData(m_rebirthData.stepOnEffect, ProcessState.Pause);
	}
	...
	public override void RebirthStartByteGame(byte[] rd_data)
	{
		...
		m_initEffect.SetParticleData(m_rebirthData.initEffect, ProcessState.UnPause);
		m_triggerEffect.SetParticleData(m_rebirthData.triggerEffect, ProcessState.UnPause);
		m_motionEffect.SetParticleData(m_rebirthData.motionEffect, ProcessState.UnPause);
		m_stepOnEffect.SetParticleData(m_rebirthData.stepOnEffect, ProcessState.UnPause);
		m_isIgnoreTrigger = false;
	}
