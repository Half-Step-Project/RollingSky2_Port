...
public class DynamicBone : MonoBehaviour
{
	...
	{
		public string m_RootName = string.Empty;
	...
	private void SetupParticlesDontClear()
	{
		if (!(m_Root == null))
		{
			m_LocalGravity = m_Root.InverseTransformDirection(m_Gravity);
	...
	private void ApplyParticlesToTransforms()
	{
		for (int i = 1; i < m_Particles.Count; i++)
		{
			...
			if (particle2.m_Transform.childCount <= 1)
			{
				...
				Vector3 normalized = particle2.m_Transform.TransformDirection(direction).normalized;
				Vector3 normalized2 = vector.normalized;
				Quaternion rotation = Quaternion.FromToRotation(normalized, normalized2) * particle2.m_Transform.rotation;
				particle2.m_Transform.rotation = rotation;
			}
			if (particle.m_Transform != null)
			{
	...
	public DynamicBoneData GetDynamicBoneData()
	{
		return new DynamicBoneData
		{
			m_RootName = ((m_Root != null) ? m_Root.gameObject.name : string.Empty),
			m_UpdateRate = m_UpdateRate,
			m_UpdateMode = m_UpdateMode,
			m_Damping = m_Damping,
			m_DampingDistrib = m_DampingDistrib,
			m_Elasticity = m_Elasticity,
			m_ElasticityDistrib = m_ElasticityDistrib,
			m_Stiffness = m_Stiffness,
			m_StiffnessDistrib = m_StiffnessDistrib,
			m_Inert = m_Inert,
			m_InertDistrib = m_InertDistrib,
			m_Radius = m_Radius,
			m_RadiusDistrib = m_RadiusDistrib,
			m_EndLength = m_EndLength,
			m_EndOffset = m_EndOffset,
			m_Gravity = m_Gravity,
			m_Force = m_Force
		};
	}

	public void SetDynamicBoneData(DynamicBoneData data, bool isSetRootTransfrom = false)
	{
		if (isSetRootTransfrom)
		{
			m_Root = (from c in base.gameObject.GetComponentsInChildren<Transform>()
				where c.name == data.m_RootName
				select c).FirstOrDefault();
		}
		m_UpdateMode = data.m_UpdateMode;
		if (Application.isPlaying)
		{
			m_UpdateRate = ((data.m_UpdateMode == UpdateMode.Normal) ? ((float)Application.targetFrameRate) : data.m_UpdateRate);
		}
		else
		{
			m_UpdateRate = data.m_UpdateRate;
		}
		m_Damping = data.m_Damping;
		m_DampingDistrib = data.m_DampingDistrib;
		m_Elasticity = data.m_Elasticity;
		m_ElasticityDistrib = data.m_ElasticityDistrib;
		m_Stiffness = data.m_Stiffness;
		m_StiffnessDistrib = data.m_StiffnessDistrib;
		m_Inert = data.m_Inert;
		m_InertDistrib = data.m_InertDistrib;
		m_Radius = data.m_Radius;
		m_RadiusDistrib = data.m_RadiusDistrib;
		m_EndLength = data.m_EndLength;
		m_EndOffset = data.m_EndOffset;
