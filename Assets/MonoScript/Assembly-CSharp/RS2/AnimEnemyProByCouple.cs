...
namespace RS2
{
	public class AnimEnemyProByCouple : BaseEnemy
	{
		[Serializable]
		...
		{
			float num = 0f;
			num = base.groupTransform.InverseTransformPoint(BaseCouple.theCouple.transform.position).z - base.groupTransform.InverseTransformPoint(base.transform.position).z;
			if (commonState == CommonState.None && BaseCouple.theCouple.IfHaveTreasureChest)
			{
