...
public static class MathUtils
{
	...
	public class RangeNullable
	{
		...
		public float Average
		{
			get
			{
				...
				return Average(Min.Value, Max.Value);
			}
		}
		...
		public float Distance
		{
			get
			{
				...
				return Distance(Min.Value, Max.Value);
			}
		}
		...
		public float ToPercent(float value)
		{
			...
			return MathUtils.ToPercent(value, Min.Value, Max.Value);
		}
		...
		public float ToPercent(float value, bool inverse)
		{
			...
			return MathUtils.ToPercent(value, Min.Value, Max.Value, inverse);
		}
		...
		public float FromPercent(float percent)
		{
			...
			return MathUtils.FromPercent(percent, Min.Value, Max.Value);
		}
		...
		public float FromPercent(float percent, bool inverse)
		{
			...
			return MathUtils.FromPercent(percent, Min.Value, Max.Value, inverse);
		}
		...
		public string ToString(bool includeLength)
		{
			...
			string arg = (Min.HasValue ? Min.ToString() : "Null");
			string arg2 = (Max.HasValue ? Max.ToString() : "Null");
	...
	public class RangePair
	{
		...
		{
			if (rangeX.Contains(queryValues.x))
			{
				return rangeY.Contains(queryValues.y);
			}
			return false;
		}
		...
		{
			if (rangeX.Contains(queryValueX))
			{
		...
		{
			if (rangeX.ContainsNot(queryValues.x))
			{
				return rangeY.ContainsNot(queryValues.y);
			}
			return false;
		}
		...
		{
			if (rangeX.ContainsNot(queryValueX))
			{
		...
		public bool DoesBorder(Vector2 values, bool requireBoth = false)
		{
			if (requireBoth)
			{
				if (rangeX.DoesBorder(values.x))
				{
					return rangeY.DoesBorder(values.y);
				}
				return false;
			}
			if (!rangeX.DoesBorder(values.x))
		...
		public bool DoesBorder(float valueX, float valueY, bool requireBoth = false)
		{
			if (requireBoth)
			{
				if (rangeX.DoesBorder(valueX))
				{
					return rangeY.DoesBorder(valueY);
				}
				return false;
			}
			if (!rangeX.DoesBorder(valueX))
	...
	public struct IntPair
	{
		...
		{
			if ((float)intPair.x == xy.x)
			{
				return (float)intPair.y == xy.y;
			}
			return false;
		}
		...
		{
			if ((float)intPair.x == xyz.x)
			{
				return (float)intPair.y == xyz.y;
			}
			return false;
		}
	...
	public struct IntTrio
	{
		...
		{
			if (x == otherIntTrio.x && y == otherIntTrio.y)
			{
				return z == otherIntTrio.z;
			}
			return false;
		}
		...
		{
			if ((float)x == xyz.x && (float)y == xyz.y)
			{
				return (float)z == xyz.z;
			}
			return false;
		}
		...
		{
			if ((float)intTrio.x == xy.x)
			{
				return (float)intTrio.y == xy.y;
			}
			return false;
		}
		...
		{
			if ((float)intTrio.x == xyz.x && (float)intTrio.y == xyz.y)
			{
				return (float)intTrio.z == xyz.z;
			}
			return false;
		}
	...
	private static TimeUnit[] timeUnits = new TimeUnit[10]
	{
		...
		new TimeUnit(1, "eternity", 2.14748365E+09f)
	};
	...
	public static float SmoothedDeltaTime
	{
		get
		{
			...
			if (!(smoothDeltaTime > frameRateMinimum.deltaTime))
			{
				return smoothDeltaTime;
			}
			return frameRateMinimum.deltaTime;
		}
	}
	...
	public static float Distance(float firstX, float secondX, float firstY, float secondY, float firstZ, float secondZ)
	{
		float firstSide = Distance(firstX, secondX);
		float secondSide = Distance(firstY, secondY);
		return Hypotenuse(secondSide: Distance(firstZ, secondZ), firstSide: Hypotenuse(firstSide, secondSide));
	}
	...
	{
		if (!IsSmaller(firstFloat, secondFloat))
		{
			return secondFloat;
		}
		return firstFloat;
	...
	{
		if (!IsLarger(firstFloat, secondFloat))
		{
			return secondFloat;
		}
		return firstFloat;
	...
	{
		if (!IsSmallerInt(firstInt, secondInt))
		{
			return secondInt;
		}
		return firstInt;
	...
	{
		if (!IsLargerInt(firstInt, secondInt))
		{
			return secondInt;
		}
		return firstInt;
	...
	public static int HalfIntRounded(int value, bool roundDown)
	{
		...
		return Rounded((float)value / 2f, roundDown);
	}
	...
	public static string FractionStringInt(int value, int divider)
	{
		...
		else
		{
			...
			else
			{
				...
				int num2 = value % divider;
				if (num2 > 0)
				{
					text = string.Format("{0} {1}", text, FractionStringInt(num2, divider));
				}
			}
			if (flag2)
			{
	...
	public static bool IndexIsWithin<ArrayType>(int index, ArrayType[] array)
	{
		if (array != null)
		{
			return IsWithin(index, 0, array.Length - 1);
		}
		return false;
	}
	...
	public static bool IndexIsNotWithin<ArrayType>(int index, ArrayType[] array)
	{
		if (array != null)
		{
			return !IndexIsWithin(index, array);
		}
		return false;
	}
	...
	private static bool TryGetTimeSince(DateTime? givenDateTime, bool getInMinutes, out int totalSince)
	{
		...
		{
			totalSince = (getInMinutes ? MinutesSince(givenDateTime) : SecondsSince(givenDateTime));
			return true;
		}
		totalSince = -1;
		return false;
	...
	private static bool TryGetTimeSince(string givenDateTimeString, bool getInMinutes, out int totalSince)
	{
		...
		{
			totalSince = (getInMinutes ? MinutesSince(result) : SecondsSince(result));
			return true;
		}
		totalSince = -1;
		return false;
	...
	public static string ToPriceString(float price, bool cents, bool spaced)
	{
		...
		else
		{
			...
			{
				float num2 = (int)(price * 100f);
				text = ((!spaced) ? (num2 + "c") : (num2 + " c"));
			}
			if (num)
			{
	...
	public static string StringFromNullable(float? nullableFloat, string valueIfNull = "NULL", string stringFormat = "0.00")
	{
		...
		return nullableFloat.Value.ToString(stringFormat);
	}
	...
	public static float ToZollyDistance(float fieldOfView, float startingFieldOfView, float multIfLesserThanStarting, float multIfGreaterThanStarting)
	{
		...
		float num2 = ((fieldOfView < startingFieldOfView) ? multIfLesserThanStarting : multIfGreaterThanStarting);
		return num * num2;
	}
